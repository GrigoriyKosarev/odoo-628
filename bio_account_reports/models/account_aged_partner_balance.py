...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        def build_result_dict(report, query_res_lines):
            ...
            if current_groupby == 'id':
                ...
                rslt.update({
                    ...
                    'invoice_date': query_res['invoice_date'][0] if len(query_res['invoice_date']) == 1 else None,   # ODOO-372
                    'bio_factoring': query_res['bio_factoring'][0] if len(query_res['bio_factoring']) == 1 else None,  # ODOO-476
                    # 'bio_factoring': self._get_bio_factoring(query_res['bio_factoring'][0]) if len(query_res['bio_factoring']) == 1 else 'No',
                ...
                rslt.update({
                    ...
                    'currency': None,
                    'account_name': None,
                    'invoice_date': None,  # ODOO-372
                    'bio_factoring': None,  # ODOO-476
                    'expected_date': None,
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT account_move.invoice_date) AS invoice_date,
                ARRAY_AGG(DISTINCT account_move.bio_factoring) AS bio_factoring,
                --ARRAY_AGG(DISTINCT CASE WHEN account_move.bio_factoring = TRUE THEN 'Yes' ELSE 'No' END) AS bio_factoring,
    ...
    def _get_bio_factoring(self, bio_factoring):
        return "Yes" if bio_factoring else "No"
...
class AccountReport(models.Model):
    ...
    filter_bio_factoring = fields.Boolean(
        string="Factoring",
        default=False,
    )

    ####################################################
    # OPTIONS: bio_factoring
    ####################################################

    def _init_options_bio_factoring(self, options, previous_options=None):

        if not self.filter_bio_factoring:
            return

        options['bio_factoring'] = [
            {'id': 'enabled', 'name': _("Enabled"), 'selected': True},
            {'id': 'disabled', 'name': _("Disabled"), 'selected': True},
        ]

        if previous_options and previous_options.get('bio_factoring'):
            previously_selected_ids = {x['id'] for x in previous_options['bio_factoring'] if x.get('selected')}
            for opt in options['bio_factoring']:
                opt['selected'] = opt['id'] in previously_selected_ids

        selected_options = {x['id']: x['name'] for x in options['bio_factoring'] if x['selected']}
        ...
        check_if_name_applicable({'enabled', 'disabled'}, _("All"))

        for sel in selected_ids:
            display_names.append(selected_options.get(sel))
        options['bio_factoring_display_name'] = ', '.join(display_names)

    def _get_options_domain(self, options, date_scope):
        domain = super()._get_options_domain(options, date_scope)

        if 'bio_factoring' in options and options['bio_factoring'] is not None:
            selected_values = [el.get('id') for el in options['bio_factoring'] if el.get('selected')]

            if 'enabled' in selected_values and 'disabled' in selected_values:
                pass
            elif 'enabled' in selected_values:
                custom_domain = [('move_id.bio_factoring', '=', True)]
                domain = expression.AND([custom_domain, domain])
            elif 'disabled' in selected_values:
                custom_domain = [('move_id.bio_factoring', '=', False)]
                domain = expression.AND([custom_domain, domain])
